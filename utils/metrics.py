...
class COMAP_Loss(nn.Module):
    ...
    def forward(self, inputs, target, use_mask=True):
        mask = target[:, 0, ...]
        if use_mask:
            mask = torch.gt(mask, torch.tensor(0)) *1.
        else:
...
def mse_loss(vertex_pred, vertex_targets, mask, scale = 4, normalize=True, reduction=True):
    """
    :param vertex_pred: [b, vn*2, h, w]
    :param vertex_targets: [b, vn*2, h, w]
    :param mask: [b, 1, h, w]
    :param scale: scaling factor
    :param normalize: whether to normalize the loss
    :param reduction: whether to reduce the loss to a single value
    :return: the MSE loss
    """
    b, ver_dim, *_ = vertex_pred.shape
    in_loss = (mask * (vertex_pred - vertex_targets)) ** 2 / scale
    if normalize:
