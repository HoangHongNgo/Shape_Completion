...
def read_diff(path):
    data = np.load(path, encoding='bytes', allow_pickle=True)
    return data['arr_0']
...
if __name__=='__main__':
    ...
    parser.add_argument("-checkpoints", type=str, default="checkpoints/SMSCM_norm_begin_re/SMSCM_norm_begin_re_checkpoint_0055.pt", help="checkpoint weight file")
    ...
    for index in range(len(rgbpath)):
        ...
        # Clip depth values to the min-max range to handle outliers
        depth_clipped = np.clip(depth, 168, 733)
        
        # Normalize to [0, 1] range
        depth_norm = (depth_clipped - 168) / (733 - 168)
        ...
        rgbd = np.append(rgb, np.expand_dims(depth_norm, axis=2), axis=2)/1.
        if args.PLUS:
